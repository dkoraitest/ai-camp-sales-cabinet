// ЭТАЛОН · оценки всех контактов (scripts/score_all.py) + инсайты по четырём разрезам.
// scope:"base" — выводы, которые появляются на шаге 4, после разбора всей базы.

window.CABINET_SCORES = {
  "items": [
    {
      "id": "c143",
      "type": "call",
      "total": 2.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l042",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c117",
      "type": "call",
      "total": 8.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": true,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c042",
      "type": "call",
      "total": 4.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 6,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c112",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c196",
      "type": "call",
      "total": 5.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l061",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c078",
      "type": "call",
      "total": 5.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c141",
      "type": "call",
      "total": 4.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l041",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c011",
      "type": "call",
      "total": 3.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l006",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c006",
      "type": "call",
      "total": 8.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l003",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c199",
      "type": "call",
      "total": 4.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l062",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c142",
      "type": "call",
      "total": 3.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l041",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c197",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l061",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c047",
      "type": "call",
      "total": 5.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c043",
      "type": "call",
      "total": 2.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c193",
      "type": "call",
      "total": 6.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l058",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c113",
      "type": "call",
      "total": 3.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c118",
      "type": "call",
      "total": 9.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l034",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c101",
      "type": "call",
      "total": 5.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c054",
      "type": "call",
      "total": 8.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c012",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l006",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c037",
      "type": "call",
      "total": 6.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 6,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l012",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c044",
      "type": "call",
      "total": 3.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c194",
      "type": "call",
      "total": 3.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l059",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c007",
      "type": "call",
      "total": 9.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l003",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c200",
      "type": "call",
      "total": 2.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l062",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c079",
      "type": "call",
      "total": 5.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c182",
      "type": "call",
      "total": 5.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l055",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c140",
      "type": "call",
      "total": 7.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l040",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c041",
      "type": "call",
      "total": 6.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l013",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c048",
      "type": "call",
      "total": 5.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c065",
      "type": "call",
      "total": 8.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l019",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c198",
      "type": "call",
      "total": 4.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l061",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c114",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c049",
      "type": "call",
      "total": 8.7,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c045",
      "type": "call",
      "total": 4.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c119",
      "type": "call",
      "total": 9.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l034",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c183",
      "type": "call",
      "total": 3.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l055",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c036",
      "type": "call",
      "total": 3.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l011",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c050",
      "type": "call",
      "total": 8.7,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c172",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l052",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c055",
      "type": "call",
      "total": 8.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c069",
      "type": "call",
      "total": 7.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c038",
      "type": "call",
      "total": 4.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l012",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c046",
      "type": "call",
      "total": 3.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c102",
      "type": "call",
      "total": 8.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c130",
      "type": "call",
      "total": 3.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c184",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l055",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c124",
      "type": "call",
      "total": 8.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c056",
      "type": "call",
      "total": 9.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c080",
      "type": "call",
      "total": 5.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c023",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c051",
      "type": "call",
      "total": 9.0,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c039",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l012",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c020",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l008",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c115",
      "type": "call",
      "total": 3.7,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c120",
      "type": "call",
      "total": 5.9,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c173",
      "type": "call",
      "total": 2.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l052",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c057",
      "type": "call",
      "total": 9.6,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c081",
      "type": "call",
      "total": 5.3,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c008",
      "type": "call",
      "total": 4.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 6,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l004",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c052",
      "type": "call",
      "total": 8.1,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l015",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c070",
      "type": "call",
      "total": 7.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c024",
      "type": "call",
      "total": 4.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c169",
      "type": "call",
      "total": 5.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l051",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c103",
      "type": "call",
      "total": 9.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c121",
      "type": "call",
      "total": 9.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l034",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c195",
      "type": "call",
      "total": 2.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l060",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c021",
      "type": "call",
      "total": 3.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l008",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c125",
      "type": "call",
      "total": 9.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c025",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c116",
      "type": "call",
      "total": 2.9,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c131",
      "type": "call",
      "total": 3.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c040",
      "type": "call",
      "total": 4.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l012",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c058",
      "type": "call",
      "total": 7.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c174",
      "type": "call",
      "total": 6.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l052",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c022",
      "type": "call",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l008",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c060",
      "type": "call",
      "total": 3.4,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c089",
      "type": "call",
      "total": 8.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l025",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c082",
      "type": "call",
      "total": 5.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c059",
      "type": "call",
      "total": 6.4,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l017",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c158",
      "type": "call",
      "total": 8.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l048",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c148",
      "type": "call",
      "total": 2.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c123",
      "type": "call",
      "total": 3.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l035",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c175",
      "type": "call",
      "total": 5.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l052",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c071",
      "type": "call",
      "total": 7.7,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c170",
      "type": "call",
      "total": 5.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l051",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c181",
      "type": "call",
      "total": 7.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 6,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l054",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c090",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l025",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c009",
      "type": "call",
      "total": 5.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l004",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c104",
      "type": "call",
      "total": 9.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c122",
      "type": "call",
      "total": 9.6,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c144",
      "type": "call",
      "total": 9.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l043",
      "bant": {
        "budget": false,
        "authority": true,
        "need": false,
        "timing": true
      }
    },
    {
      "id": "c105",
      "type": "call",
      "total": 9.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c126",
      "type": "call",
      "total": 9.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c072",
      "type": "call",
      "total": 6.6,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c083",
      "type": "call",
      "total": 2.7,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c159",
      "type": "call",
      "total": 6.3,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l048",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c100",
      "type": "call",
      "total": 9.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l029",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c132",
      "type": "call",
      "total": 3.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c108",
      "type": "call",
      "total": 8.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l032",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c026",
      "type": "call",
      "total": 4.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c066",
      "type": "call",
      "total": 9.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l020",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c053",
      "type": "call",
      "total": 3.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l016",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c160",
      "type": "call",
      "total": 8.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l049",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c164",
      "type": "call",
      "total": 3.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c133",
      "type": "call",
      "total": 5.3,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c145",
      "type": "call",
      "total": 9.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l043",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c147",
      "type": "call",
      "total": 4.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l044",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c139",
      "type": "call",
      "total": 6.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l039",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c185",
      "type": "call",
      "total": 9.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c061",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c149",
      "type": "call",
      "total": 3.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c171",
      "type": "call",
      "total": 6.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l051",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c106",
      "type": "call",
      "total": 9.1,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l030",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c146",
      "type": "call",
      "total": 8.9,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l043",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c067",
      "type": "call",
      "total": 9.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l020",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c075",
      "type": "call",
      "total": 2.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l022",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c027",
      "type": "call",
      "total": 3.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c085",
      "type": "call",
      "total": 9.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l024",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c127",
      "type": "call",
      "total": 8.9,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c091",
      "type": "call",
      "total": 9.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l026",
      "bant": {
        "budget": true,
        "authority": true,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c176",
      "type": "call",
      "total": 6.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l053",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c062",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c153",
      "type": "call",
      "total": 3.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l046",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c150",
      "type": "call",
      "total": 5.4,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c073",
      "type": "call",
      "total": 6.9,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c128",
      "type": "call",
      "total": 8.7,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c084",
      "type": "call",
      "total": 2.7,
      "stage": "closed_lost",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c092",
      "type": "call",
      "total": 7.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l026",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c109",
      "type": "call",
      "total": 8.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l032",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c134",
      "type": "call",
      "total": 3.7,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c165",
      "type": "call",
      "total": 5.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c093",
      "type": "call",
      "total": 4.0,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c186",
      "type": "call",
      "total": 8.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c161",
      "type": "call",
      "total": 7.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l049",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c191",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l057",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c028",
      "type": "call",
      "total": 3.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c177",
      "type": "call",
      "total": 6.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l053",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c010",
      "type": "call",
      "total": 3.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l005",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c086",
      "type": "call",
      "total": 9.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l024",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c076",
      "type": "call",
      "total": 2.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l022",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c166",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c162",
      "type": "call",
      "total": 9.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l049",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c013",
      "type": "call",
      "total": 4.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c154",
      "type": "call",
      "total": 2.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l046",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c087",
      "type": "call",
      "total": 7.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l024",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c136",
      "type": "call",
      "total": 6.1,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l038",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c156",
      "type": "call",
      "total": 3.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l047",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c001",
      "type": "call",
      "total": 3.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l001",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c192",
      "type": "call",
      "total": 8.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l057",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c063",
      "type": "call",
      "total": 4.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c107",
      "type": "call",
      "total": 3.6,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l031",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c110",
      "type": "call",
      "total": 9.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l032",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c068",
      "type": "call",
      "total": 8.4,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован",
      "lead_id": "l020",
      "bant": {
        "budget": true,
        "authority": false,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c129",
      "type": "call",
      "total": 9.6,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l036",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c178",
      "type": "call",
      "total": 5.9,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l053",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c094",
      "type": "call",
      "total": 2.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c077",
      "type": "call",
      "total": 5.7,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l022",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c074",
      "type": "call",
      "total": 3.3,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c187",
      "type": "call",
      "total": 9.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c163",
      "type": "call",
      "total": 8.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l049",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c030",
      "type": "call",
      "total": 8.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l010",
      "bant": {
        "budget": true,
        "authority": true,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c088",
      "type": "call",
      "total": 9.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l024",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c151",
      "type": "call",
      "total": 5.4,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c005",
      "type": "call",
      "total": 4.7,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l002",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c097",
      "type": "call",
      "total": 4.9,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l028",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c152",
      "type": "call",
      "total": 6.7,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c095",
      "type": "call",
      "total": 5.4,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c031",
      "type": "call",
      "total": 7.3,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c029",
      "type": "call",
      "total": 5.6,
      "stage": "closed_lost",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 6,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c01",
      "type": "call",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "перешли к демо без выявленной боли",
      "lead_id": "l03",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c157",
      "type": "call",
      "total": 6.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l047",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c02",
      "type": "call",
      "total": 7.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "выявлена боль, назначена встреча с финдиректором",
      "lead_id": "l05",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c135",
      "type": "call",
      "total": 3.3,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c064",
      "type": "call",
      "total": 5.3,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c03",
      "type": "call",
      "total": 3.4,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "демо проведено, следующий шаг не зафиксирован",
      "lead_id": "l07",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c167",
      "type": "call",
      "total": 5.4,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c188",
      "type": "call",
      "total": 9.6,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c014",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c04",
      "type": "call",
      "total": 7.3,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "КП отправлено, договорённость о решении к 12.09",
      "lead_id": "l05",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": true
      }
    },
    {
      "id": "c05",
      "type": "call",
      "total": 4.3,
      "stage": "first_contact",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "клиент не понял, кто звонит, разговор свернулся",
      "lead_id": "l11",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c098",
      "type": "call",
      "total": 3.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l028",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c06",
      "type": "call",
      "total": 4.3,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "дана скидка 15% без встречного условия",
      "lead_id": "l09",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c137",
      "type": "call",
      "total": 3.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l038",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c155",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l046",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c168",
      "type": "call",
      "total": 3.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c189",
      "type": "call",
      "total": 9.6,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c111",
      "type": "call",
      "total": 8.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l032",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c179",
      "type": "call",
      "total": 7.3,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l053",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c096",
      "type": "call",
      "total": 5.7,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c032",
      "type": "call",
      "total": 5.4,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c002",
      "type": "call",
      "total": 4.6,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l001",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c07",
      "type": "call",
      "total": 3.1,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "договор подписан, 8.4 млн ₸",
      "lead_id": "l02",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c08",
      "type": "call",
      "total": 4.0,
      "stage": "closed_lost",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "клиент выбрал конкурента",
      "lead_id": "l03",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c09",
      "type": "call",
      "total": 5.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 5,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 8,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность частично выявлена, ЛПР не установлен",
      "lead_id": "l14",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c180",
      "type": "call",
      "total": 5.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l053",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c138",
      "type": "call",
      "total": 3.6,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l038",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c10",
      "type": "call",
      "total": 7.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "демо под сценарий клиента, назначено КП",
      "lead_id": "l16",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c11",
      "type": "call",
      "total": 7.7,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "выявлены три стейкхолдера, согласован формат пилота",
      "lead_id": "l18",
      "bant": {
        "budget": false,
        "authority": true,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c12",
      "type": "call",
      "total": 3.6,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "follow-up без результата, next step не назначен",
      "lead_id": "l07",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c099",
      "type": "call",
      "total": 4.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l028",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c015",
      "type": "call",
      "total": 5.3,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c033",
      "type": "call",
      "total": 8.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c190",
      "type": "call",
      "total": 9.0,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой",
      "lead_id": "l056",
      "bant": {
        "budget": true,
        "authority": true,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c003",
      "type": "call",
      "total": 7.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l001",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c034",
      "type": "call",
      "total": 4.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c035",
      "type": "call",
      "total": 5.1,
      "stage": "closed_won",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c016",
      "type": "call",
      "total": 6.9,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c004",
      "type": "call",
      "total": 3.6,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован",
      "lead_id": "l001",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "c017",
      "type": "call",
      "total": 6.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "c018",
      "type": "call",
      "total": 7.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": true,
        "need": true,
        "timing": true
      }
    },
    {
      "id": "c019",
      "type": "call",
      "total": 5.4,
      "stage": "closed_lost",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": true,
        "timing": false
      }
    },
    {
      "id": "ch039",
      "type": "chat",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l033",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch002",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l003",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch013",
      "type": "chat",
      "total": 4.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch040",
      "type": "chat",
      "total": 5.4,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch014",
      "type": "chat",
      "total": 4.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch003",
      "type": "chat",
      "total": 5.3,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l006",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch026",
      "type": "chat",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch016",
      "type": "chat",
      "total": 5.4,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l015",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch041",
      "type": "chat",
      "total": 6.3,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l034",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch017",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l017",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch015",
      "type": "chat",
      "total": 4.0,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l014",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch034",
      "type": "chat",
      "total": 5.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l030",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch042",
      "type": "chat",
      "total": 5.4,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch012",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l012",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch055",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l052",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch022",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch035",
      "type": "chat",
      "total": 6.9,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l030",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch043",
      "type": "chat",
      "total": 5.4,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l034",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch006",
      "type": "chat",
      "total": 4.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l008",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch044",
      "type": "chat",
      "total": 5.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l036",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch027",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l023",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch030",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l025",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch046",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch054",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l051",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch048",
      "type": "chat",
      "total": 5.3,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l043",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch007",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch018",
      "type": "chat",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch049",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l045",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch023",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch045",
      "type": "chat",
      "total": 6.3,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l036",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch008",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l009",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch028",
      "type": "chat",
      "total": 6.9,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l024",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch036",
      "type": "chat",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l032",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch024",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l021",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch031",
      "type": "chat",
      "total": 4.0,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch047",
      "type": "chat",
      "total": 5.9,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l037",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch019",
      "type": "chat",
      "total": 4.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch025",
      "type": "chat",
      "total": 4.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l022",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch052",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch02",
      "type": "chat",
      "total": 5.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "снято скрытое возражение финдиректора, решение ускорено",
      "lead_id": "l05",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch01",
      "type": "chat",
      "total": 4.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "переписка затухла, менеджер не перехватил инициативу",
      "lead_id": "l07",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch029",
      "type": "chat",
      "total": 5.4,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l024",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch03",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг по скидке продолжен в переписке, ценность не возвращена в разговор",
      "lead_id": "l09",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch053",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l050",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch037",
      "type": "chat",
      "total": 5.9,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l032",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch020",
      "type": "chat",
      "total": 4.0,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l018",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch009",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch021",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l020",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch050",
      "type": "chat",
      "total": 6.9,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 10,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено",
      "lead_id": "l045",
      "bant": {
        "budget": true,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch04",
      "type": "chat",
      "total": 3.1,
      "stage": "discovery",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс отправлен без контекста, диалог закончился",
      "lead_id": "l14",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch051",
      "type": "chat",
      "total": 3.1,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился",
      "lead_id": "l046",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch032",
      "type": "chat",
      "total": 4.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l027",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch038",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l032",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch010",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch033",
      "type": "chat",
      "total": 4.0,
      "stage": "demo",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l028",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch011",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l010",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch004",
      "type": "chat",
      "total": 3.1,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch001",
      "type": "chat",
      "total": 4.0,
      "stage": "proposal",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла",
      "lead_id": "l001",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    },
    {
      "id": "ch005",
      "type": "chat",
      "total": 3.1,
      "stage": "negotiation",
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась",
      "lead_id": "l007",
      "bant": {
        "budget": false,
        "authority": false,
        "need": false,
        "timing": false
      }
    }
  ],
  "aggregates": {
    "profile": "b2b",
    "total_contacts": 271,
    "calls": 212,
    "chats": 59,
    "focus_stage": "discovery",
    "focus_label": "Выявление потребности",
    "qualification_stage": "discovery",
    "avg_total": 5.5,
    "focus": {
      "contacts": 68,
      "avg": 5.2,
      "with_depth": 13,
      "with_close": 27,
      "pitch_first": 8,
      "avg_q_before_pitch": 4.2,
      "by_manager": {
        "Виктория Ким": 7.3,
        "Денис Ахметов": 3.8,
        "Марат Сулейменов": 4.0,
        "Айдана Бекова": 4.5
      }
    },
    "funnel": [
      {
        "stage": "first_contact",
        "label": "Первый контакт",
        "contacts": 63,
        "avg": 5.7,
        "leads_on_stage": 17,
        "money_on_stage": 146750000,
        "close_rate": 0.52
      },
      {
        "stage": "discovery",
        "label": "Выявление потребности",
        "contacts": 68,
        "avg": 5.2,
        "leads_on_stage": 12,
        "money_on_stage": 104400000,
        "close_rate": 0.4
      },
      {
        "stage": "demo",
        "label": "Демонстрация",
        "contacts": 50,
        "avg": 5.6,
        "leads_on_stage": 11,
        "money_on_stage": 100150000,
        "close_rate": 0.4
      },
      {
        "stage": "proposal",
        "label": "КП",
        "contacts": 43,
        "avg": 5.5,
        "leads_on_stage": 9,
        "money_on_stage": 75750000,
        "close_rate": 0.44
      },
      {
        "stage": "negotiation",
        "label": "Переговоры",
        "contacts": 33,
        "avg": 5.3,
        "leads_on_stage": 7,
        "money_on_stage": 81800000,
        "close_rate": 0.48
      },
      {
        "stage": "closed_won",
        "label": "Сделка закрыта",
        "contacts": 10,
        "avg": 6.7,
        "leads_on_stage": 10,
        "money_on_stage": 70900000,
        "close_rate": 0.7
      },
      {
        "stage": "closed_lost",
        "label": "Проиграна",
        "contacts": 4,
        "avg": 4.4,
        "leads_on_stage": 4,
        "money_on_stage": 41900000,
        "close_rate": 0.25
      }
    ],
    "channels": {
      "calls_avg": 5.9,
      "chats_avg": 4.1
    },
    "managers": {
      "Виктория Ким": {
        "contacts": 88,
        "avg": 7.7,
        "close_rate": 0.89,
        "depth_rate": 0.59
      },
      "Денис Ахметов": {
        "contacts": 65,
        "avg": 4.0,
        "close_rate": 0.2,
        "depth_rate": 0.0
      },
      "Марат Сулейменов": {
        "contacts": 70,
        "avg": 4.2,
        "close_rate": 0.17,
        "depth_rate": 0.0
      },
      "Айдана Бекова": {
        "contacts": 48,
        "avg": 5.3,
        "close_rate": 0.42,
        "depth_rate": 0.17
      }
    },
    "behaviour_vs_outcome": {
      "with_depth": 0.48,
      "without_depth": 0.41,
      "with_close": 0.52,
      "without_close": 0.35,
      "objection_counter": 0.7,
      "objection_discount": 0.47
    },
    "bant": {
      "stage": "discovery",
      "label": "Выявление потребности",
      "contacts": 68,
      "coverage": {
        "budget": 0.1,
        "authority": 0.15,
        "need": 0.35,
        "timing": 0.15
      },
      "full": 3,
      "none": 41,
      "full_vs_outcome": 0.67
    },
    "segments": [
      {
        "segment": "Дистрибуция",
        "leads": 65,
        "contacts": 118,
        "won": 10,
        "lost": 7,
        "win_rate": 0.59,
        "avg_quality": 5.8,
        "top_client_lines": [
          "Сборка заказа занимает полтора часа вместо двадцати минут.",
          "Похожие позиции путают постоянно, возвраты каждую неделю.",
          "Решаю я, но ИТ должен посмотреть на интеграции.",
          "Да, но недолго, у меня встреча через пятнадцать минут.",
          "Я инициирую, но бюджет утверждает финансовый директор."
        ]
      },
      {
        "segment": "Логистика",
        "leads": 55,
        "contacts": 84,
        "won": 6,
        "lost": 2,
        "win_rate": 0.75,
        "avg_quality": 5.9,
        "top_client_lines": [
          "Отгрузки срываются, клиенты ждут по две-три недели.",
          "Три склада: основной в Алматы и два региональных.",
          "Остатки не сходятся с фактом, находим только на инвентаризации.",
          "Я инициирую, но бюджет утверждает финансовый директор.",
          "Пока не закладывали, будем смотреть по цифрам."
        ]
      },
      {
        "segment": "Производство",
        "leads": 39,
        "contacts": 60,
        "won": 4,
        "lost": 3,
        "win_rate": 0.57,
        "avg_quality": 4.3,
        "top_client_lines": [
          "Сырьё списывается задним числом, себестоимость плывёт.",
          "Директор видит картину по группе раз в месяц и уже неактуальную.",
          "Да, но недолго, у меня встреча через пятнадцать минут.",
          "У нас уже стоит система, правда толком не пользуемся.",
          "Мы можем сначала на одном направлении попробовать?"
        ]
      },
      {
        "segment": "прочие (13 мелких)",
        "leads": 15,
        "contacts": 9,
        "won": 0,
        "lost": 3,
        "win_rate": 0.0,
        "avg_quality": 5.4,
        "top_client_lines": []
      }
    ],
    "bant_by_stage": {
      "deals": 71,
      "stages": [
        {
          "stage": "first_contact",
          "label": "Первый контакт",
          "leads": 17,
          "full": 0,
          "partial": 5,
          "none": 12,
          "money": 146750000,
          "money_full": 0,
          "coverage": {
            "budget": 2,
            "authority": 3,
            "need": 5,
            "timing": 1
          }
        },
        {
          "stage": "discovery",
          "label": "Выявление потребности",
          "leads": 12,
          "full": 0,
          "partial": 9,
          "none": 3,
          "money": 104400000,
          "money_full": 0,
          "coverage": {
            "budget": 4,
            "authority": 5,
            "need": 8,
            "timing": 2
          }
        },
        {
          "stage": "demo",
          "label": "Демонстрация",
          "leads": 11,
          "full": 2,
          "partial": 2,
          "none": 7,
          "money": 100150000,
          "money_full": 12600000,
          "coverage": {
            "budget": 2,
            "authority": 2,
            "need": 4,
            "timing": 2
          }
        },
        {
          "stage": "proposal",
          "label": "КП",
          "leads": 9,
          "full": 3,
          "partial": 5,
          "none": 1,
          "money": 75750000,
          "money_full": 38950000,
          "coverage": {
            "budget": 5,
            "authority": 3,
            "need": 7,
            "timing": 4
          }
        },
        {
          "stage": "negotiation",
          "label": "Переговоры",
          "leads": 7,
          "full": 0,
          "partial": 3,
          "none": 4,
          "money": 81800000,
          "money_full": 0,
          "coverage": {
            "budget": 1,
            "authority": 0,
            "need": 3,
            "timing": 1
          }
        },
        {
          "stage": "closed_won",
          "label": "Сделка закрыта",
          "leads": 10,
          "full": 7,
          "partial": 2,
          "none": 1,
          "money": 70900000,
          "money_full": 38100000,
          "coverage": {
            "budget": 7,
            "authority": 8,
            "need": 9,
            "timing": 8
          }
        },
        {
          "stage": "closed_lost",
          "label": "Проиграна",
          "leads": 5,
          "full": 0,
          "partial": 3,
          "none": 2,
          "money": 44700000,
          "money_full": 0,
          "coverage": {
            "budget": 0,
            "authority": 1,
            "need": 3,
            "timing": 1
          }
        }
      ],
      "labels": {
        "budget": "Бюджет",
        "authority": "Полномочия",
        "need": "Потребность",
        "timing": "Сроки"
      }
    },
    "won_vs_lost": {
      "won_label": "Сделка закрыта",
      "lost_label": "Проиграна",
      "won_deals": 10,
      "lost_deals": 5,
      "won_contacts": 73,
      "lost_contacts": 30,
      "rows": [
        {
          "label": "Спросили, во что обходится проблема",
          "kind": "share",
          "won": 0.38,
          "lost": 0.0
        },
        {
          "label": "Квалифицировали в разговоре",
          "kind": "share",
          "won": 0.52,
          "lost": 0.07
        },
        {
          "label": "Зафиксировали следующий шаг",
          "kind": "share",
          "won": 0.74,
          "lost": 0.2
        },
        {
          "label": "Открыли разговор слабо",
          "kind": "share",
          "won": 0.03,
          "lost": 0.27
        },
        {
          "label": "На возражение — встречный вопрос",
          "kind": "share",
          "won": 0.38,
          "lost": 0.0
        },
        {
          "label": "На возражение — скидка",
          "kind": "share",
          "won": 0.08,
          "lost": 0.13
        },
        {
          "label": "Полный BANT по сделке",
          "kind": "share",
          "won": 0.7,
          "lost": 0.0
        },
        {
          "label": "Вопросов до первого слова о продукте",
          "kind": "num",
          "won": 6.4,
          "lost": 3.2
        },
        {
          "label": "Коммуникаций на сделку",
          "kind": "num",
          "won": 7.3,
          "lost": 6.0
        },
        {
          "label": "Средний балл разговора",
          "kind": "num",
          "won": 6.8,
          "lost": 4.4
        }
      ]
    }
  },
  "insights": [
    {
      "level": "bad",
      "title": "На этапе «Выявление потребности» разговор начинается с продукта в 8 случаях из 68",
      "text": "Это тот этап, где решается, будет ли сделка. В среднем менеджер задаёт 4.2 вопроса до первого слова о продукте, а до цифры потерь доходит только в 13 разговорах из 68. Средний балл этапа — 5.2 из 10."
    },
    {
      "level": "bad",
      "title": "Следующий шаг с датой меняет судьбу сделки: 52% против 35%",
      "text": "Там, где менеджер закончил разговор конкретной договорённостью, сделка доходит до КП и дальше в 52% случаев. Там, где всё закончилось на «я пришлю информацию» — в 35%. Самый дешёвый в исполнении приём из всех, что есть в отделе."
    },
    {
      "level": "warn",
      "title": "Разброс в команде: 7.7 против 4.0",
      "text": "Виктория Ким фиксирует следующий шаг в 89% разговоров и доводит до цифры потерь в 59%. Денис Ахметов —  20% и 0%. Разница не в старании, а в двух привычках, которым можно научить."
    },
    {
      "level": "bad",
      "title": "Переписка проседает сильнее звонков: 4.1 против 5.9",
      "text": "В чатах менеджеры напоминают о себе вместо ведения сделки. «Отправил — напоминаю — есть новости?» не даёт клиенту повода ответить. В звонке провал слышно сразу, в переписке он выглядит как вежливая тишина."
    },
    {
      "level": "good",
      "title": "Встречный вопрос на возражение работает лучше скидки: 70% против 47%",
      "text": "Когда на «дорого» менеджер спрашивает, с чем сравнивают и какой бюджет закладывали, сделка идёт дальше в полтора раза чаще, чем когда он сразу предлагает скидку. Скидка при этом ещё и съедает маржу."
    },
    {
      "level": "warn",
      "title": "Квалификация проходит полностью в 3 контактах из 68",
      "text": "На этапе «Выявление потребности» бюджет выясняют в 10% разговоров, кто принимает решение — в 15%, сроки — в 15%. Там, где выяснены все четыре признака, до КП и дальше доходит 67% сделок. В 41 контактах не выяснено ничего."
    },
    {
      "level": "bad",
      "scope": "base",
      "title": "Теряем не там, где кажется",
      "text": "По числу контактов больше всего отваливается на первом касании — это видно и без аналитики. Но в деньгах самый дорогой затор дальше, на этапе «Выявление потребности»: там стоит 12 сделок на 104 млн ₸, и каждая уже прошла через разговор, демонстрацию и расчёт. Работа с этим десятком стоит больше, чем сотня новых лидов."
    },
    {
      "level": "good",
      "scope": "base",
      "title": "Полная квалификация предсказывает сделку лучше всего остального",
      "text": "Из всех признаков, которые считает система, сильнее всего с исходом связан один: выяснены ли бюджет, ЛПР, потребность и сроки. Там, где выяснены все четыре, до КП и дальше доходит 67% сделок — против 35% у разговоров без договорённости. Значит, учить команду надо одному навыку, а не семи сразу."
    },
    {
      "level": "warn",
      "scope": "base",
      "title": "Лучший сегмент — «Логистика», и обслуживается он не лучше других",
      "text": "Доля выигранных сделок 75% при среднем качестве разговоров 5.9 из 10 — сегмент вытягивает себя сам. При нормальной работе он может дать заметно больше, и это самый дешёвый рост из доступных."
    }
  ]
};
