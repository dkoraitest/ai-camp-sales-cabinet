// ЭТАЛОН · пересчитан по фактической базе (b2b): 265 коммуникаций.
// Сгенерировано scripts/score_reference.py — цифры сходятся с данными.

window.CABINET_SCORES = {
  "items": [
    {
      "id": "c134",
      "type": "call",
      "total": 8.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c135",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c077",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c054",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c078",
      "type": "call",
      "total": 6.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c130",
      "type": "call",
      "total": 9.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c183",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c079",
      "type": "call",
      "total": 7.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c065",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c131",
      "type": "call",
      "total": 7.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c112",
      "type": "call",
      "total": 7.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c136",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c066",
      "type": "call",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c178",
      "type": "call",
      "total": 7.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c080",
      "type": "call",
      "total": 8.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c067",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c184",
      "type": "call",
      "total": 2.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c084",
      "type": "call",
      "total": 9.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c125",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c132",
      "type": "call",
      "total": 9.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c149",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c085",
      "type": "call",
      "total": 7.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c137",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c116",
      "type": "call",
      "total": 4.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c169",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c013",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c068",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c150",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c126",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c117",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c002",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c138",
      "type": "call",
      "total": 7.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c086",
      "type": "call",
      "total": 8.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c038",
      "type": "call",
      "total": 6.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c151",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c029",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c097",
      "type": "call",
      "total": 6.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c133",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c190",
      "type": "call",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c185",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c020",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c152",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c127",
      "type": "call",
      "total": 8.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c069",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c087",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c052",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c118",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c186",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c160",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c014",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c139",
      "type": "call",
      "total": 6.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c191",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c153",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c157",
      "type": "call",
      "total": 9.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c070",
      "type": "call",
      "total": 2.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c166",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c039",
      "type": "call",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c098",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c128",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c187",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c192",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c005",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c048",
      "type": "call",
      "total": 4.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c088",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c099",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c140",
      "type": "call",
      "total": 8.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c053",
      "type": "call",
      "total": 4.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c081",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c100",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c141",
      "type": "call",
      "total": 3.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c154",
      "type": "call",
      "total": 4.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c062",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c158",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c043",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c188",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c119",
      "type": "call",
      "total": 7.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c101",
      "type": "call",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c174",
      "type": "call",
      "total": 4.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c055",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c040",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c015",
      "type": "call",
      "total": 7.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c129",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c063",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c089",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c193",
      "type": "call",
      "total": 5.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c167",
      "type": "call",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c082",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c107",
      "type": "call",
      "total": 2.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c031",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c145",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c023",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c012",
      "type": "call",
      "total": 3.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c155",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c049",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c189",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c006",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c030",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c056",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c179",
      "type": "call",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c083",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c142",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c108",
      "type": "call",
      "total": 6.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c003",
      "type": "call",
      "total": 4.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c044",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c016",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c120",
      "type": "call",
      "total": 9.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c195",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c161",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c159",
      "type": "call",
      "total": 7.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c168",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c172",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c113",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c197",
      "type": "call",
      "total": 8.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c143",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c090",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c064",
      "type": "call",
      "total": 7.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c024",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c102",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c175",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c004",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c180",
      "type": "call",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c041",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c176",
      "type": "call",
      "total": 7.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c032",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c170",
      "type": "call",
      "total": 4.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c018",
      "type": "call",
      "total": 6.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c156",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c181",
      "type": "call",
      "total": 5.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c045",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c114",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c121",
      "type": "call",
      "total": 7.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c109",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c050",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c017",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c182",
      "type": "call",
      "total": 5.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c021",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c057",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c173",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c162",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c198",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c046",
      "type": "call",
      "total": 5.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c025",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c115",
      "type": "call",
      "total": 5.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c110",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c042",
      "type": "call",
      "total": 6.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c196",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c144",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c022",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c001",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c058",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c007",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c091",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c103",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c019",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c033",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c071",
      "type": "call",
      "total": 3.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c122",
      "type": "call",
      "total": 7.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, next step не зафиксирован"
    },
    {
      "id": "c104",
      "type": "call",
      "total": 4.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c171",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c163",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c026",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c092",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c146",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c177",
      "type": "call",
      "total": 7.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c111",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c034",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 3,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c194",
      "type": "call",
      "total": 2.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c008",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c01",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "перешли к демо без выявленной боли"
    },
    {
      "id": "c02",
      "type": "call",
      "total": 7.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "выявлена боль, назначена встреча с финдиректором"
    },
    {
      "id": "c047",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c027",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c051",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c03",
      "type": "call",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "демо проведено, следующий шаг не зафиксирован"
    },
    {
      "id": "c04",
      "type": "call",
      "total": 5.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "КП отправлено, договорённость о решении к 12.09"
    },
    {
      "id": "c199",
      "type": "call",
      "total": 8.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 3,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c05",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 4,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 5,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "клиент не понял, кто звонит, разговор свернулся"
    },
    {
      "id": "c072",
      "type": "call",
      "total": 2.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c105",
      "type": "call",
      "total": 4.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c06",
      "type": "call",
      "total": 4.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "дана скидка 15% без встречного условия"
    },
    {
      "id": "c123",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c009",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c059",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c073",
      "type": "call",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c07",
      "type": "call",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "договор подписан, 8.4 млн ₸"
    },
    {
      "id": "c08",
      "type": "call",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "клиент выбрал конкурента"
    },
    {
      "id": "c035",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c164",
      "type": "call",
      "total": 4.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c028",
      "type": "call",
      "total": 8.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c09",
      "type": "call",
      "total": 5.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 8,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность частично выявлена, ЛПР не установлен"
    },
    {
      "id": "c093",
      "type": "call",
      "total": 8.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c10",
      "type": "call",
      "total": 7.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 6,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "демо под сценарий клиента, назначено КП"
    },
    {
      "id": "c147",
      "type": "call",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c11",
      "type": "call",
      "total": 7.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "выявлены три стейкхолдера, согласован формат пилота"
    },
    {
      "id": "c200",
      "type": "call",
      "total": 7.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 5,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c010",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c060",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c094",
      "type": "call",
      "total": 8.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c12",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "follow-up без результата, next step не назначен"
    },
    {
      "id": "c165",
      "type": "call",
      "total": 6.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 7,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 5,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, next step не зафиксирован"
    },
    {
      "id": "c036",
      "type": "call",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c074",
      "type": "call",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c124",
      "type": "call",
      "total": 9.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 10,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 10,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 10,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 6,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "потребность раскрыта до последствий, следующий шаг с датой"
    },
    {
      "id": "c106",
      "type": "call",
      "total": 3.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c095",
      "type": "call",
      "total": 7.6,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c037",
      "type": "call",
      "total": 3.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 2,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c075",
      "type": "call",
      "total": 3.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c148",
      "type": "call",
      "total": 3.3,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 7,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c096",
      "type": "call",
      "total": 6.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 3,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "c061",
      "type": "call",
      "total": 2.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 1,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 4,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, next step не зафиксирован"
    },
    {
      "id": "c011",
      "type": "call",
      "total": 7.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 7,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 2,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 9,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "ситуация выяснена, до сути не дошли, следующий шаг с датой"
    },
    {
      "id": "c076",
      "type": "call",
      "total": 5.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 9,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "презентация без выявления, следующий шаг с датой"
    },
    {
      "id": "ch033",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch019",
      "type": "chat",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch016",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch021",
      "type": "chat",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch046",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch034",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch037",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch017",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch032",
      "type": "chat",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch038",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch031",
      "type": "chat",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch047",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch012",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch024",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch014",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch020",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch008",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch042",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch040",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch015",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch009",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch004",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch028",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch025",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch041",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch002",
      "type": "chat",
      "total": 5.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch043",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch026",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch035",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch045",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch048",
      "type": "chat",
      "total": 4.9,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch044",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch02",
      "type": "chat",
      "total": 4.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 5,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "снято скрытое возражение финдиректора, решение ускорено"
    },
    {
      "id": "ch029",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch01",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "переписка затухла, менеджер не перехватил инициативу"
    },
    {
      "id": "ch013",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch003",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг ушёл в мессенджер, ценность не вернулась"
    },
    {
      "id": "ch03",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "торг по скидке продолжен в переписке, ценность не возвращена в разговор"
    },
    {
      "id": "ch011",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch018",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch007",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch005",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch04",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс отправлен без контекста, диалог закончился"
    },
    {
      "id": "ch006",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch027",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch001",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch022",
      "type": "chat",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch010",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    },
    {
      "id": "ch030",
      "type": "chat",
      "total": 5.0,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 1,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 6,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch049",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch023",
      "type": "chat",
      "total": 5.4,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 7,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 9,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 8,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "скрытое возражение снято в переписке, решение ускорено"
    },
    {
      "id": "ch039",
      "type": "chat",
      "total": 3.1,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 0,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 1,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "прайс без контекста, диалог закончился"
    },
    {
      "id": "ch036",
      "type": "chat",
      "total": 3.7,
      "evaluate": [
        {
          "id": "structure",
          "name": "Соблюдение структуры разговора",
          "value": 5,
          "max": 10
        },
        {
          "id": "discovery",
          "name": "Глубина выявления",
          "value": 2,
          "max": 10
        },
        {
          "id": "qualification",
          "name": "Качество квалификации",
          "value": 0,
          "max": 10
        },
        {
          "id": "objections",
          "name": "Работа с ценой и возражениями",
          "value": 4,
          "max": 10
        },
        {
          "id": "deal_control",
          "name": "Управление сделкой",
          "value": 3,
          "max": 10
        },
        {
          "id": "expertise",
          "name": "Экспертность",
          "value": 3,
          "max": 10
        },
        {
          "id": "balance",
          "name": "Баланс диалога",
          "value": 9,
          "max": 10
        }
      ],
      "comment": "серия напоминаний без нового аргумента, переписка затухла"
    }
  ],
  "insights": [
    {
      "level": "bad",
      "title": "Следующий шаг фиксируется в 37% контактов",
      "text": "Из 265 разобранных коммуникаций договорённость с датой есть в 97. Остальные заканчиваются на «я пришлю информацию» и «подумайте» — именно из этой части набирается очередь зависших сделок."
    },
    {
      "level": "bad",
      "title": "Потребность доводится до цифры в 20% разговоров — и это меняет исход",
      "text": "Когда менеджер спрашивает, во что клиенту обходится текущая ситуация, сделка оказывается на КП или дальше в 50% случаев. Без такого вопроса — в 41%."
    },
    {
      "level": "warn",
      "title": "Разброс внутри команды",
      "text": "Виктория Ким 7.6 из 10, Денис Ахметов 3.9 при среднем 5.1. Это не разница в старании: она складывается из того, задаёт ли человек вопросы до презентации и фиксирует ли договорённость в конце."
    },
    {
      "level": "bad",
      "title": "Переписка слабее звонков: 3.6 против 5.5",
      "text": "В чатах менеджеры напоминают о себе вместо ведения сделки. Серия «отправил — напоминаю — есть новости?» не даёт клиенту ни одного повода ответить, и переписка затухает."
    },
    {
      "level": "warn",
      "title": "Возражение отрабатывают решением, но не все",
      "text": "Возражение прозвучало в 102 контактах. Ответом «дам скидку» или «спрошу у руководителя» закрывали 12, встречным вопросом или предложением решения — 27. Остальные ответили свойствами продукта или согласились «подумайте» — это тихая потеря сделки."
    },
    {
      "level": "warn",
      "title": "Квалификация проходит в 20% контактов",
      "text": "В остальных не выяснено, кто принимает решение и какой бюджет. Эти сделки возвращаются на предыдущий этап чаще всего: возражение приходит от человека, которого в разговоре не было."
    }
  ]
};
